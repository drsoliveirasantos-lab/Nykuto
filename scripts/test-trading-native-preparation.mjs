...
import { JEU20_PRODUCTS } from '../trading/lab/jeu20-policy.mjs';
...
const product=JEU20_PRODUCTS[0];
function fixture(){
  const scheduleEvents=new Map(),segments=JEU19_SEGMENTS.filter(d=>d.symbol==='MYM').map(d=>{
    const bars=[],native30=[];let index=0;
    for(const s of historyCalendar(d.prep,d.end)){
      scheduleEvents.set(s.date,[{product_code:'MYM',trading_venue:'XCBT',session_end_date:s.date,event:'open',timestamp:s.date+'T00:00:00Z'},{product_code:'MYM',trading_venue:'XCBT',session_end_date:s.date,event:'close',timestamp:s.date+'T23:00:00Z'}]);
      ...
      for(let i=0;i<count;i++){const price=50000+index++;if(d.ticker==='MYMM6'&&s.date==='2026-03-03'&&i===0)continue;day.push([start+i*300,price,price+4,price-8,price+2,10]);}
      bars.push(...day);
    ...
    return {ticker:d.ticker,paginationComplete:true,nativePaginationComplete:true,metadata:{ticker:d.ticker,product_code:'MYM',trading_venue:'XCBT',trade_tick_size:1,first_trade_date:'2024-01-01',last_trade_date:'2026-12-31'},bars,native30};
  });
  return {symbol:'MYM',segments,scheduleEvents:[...scheduleEvents.values()].flat()};
}
...
test('native trend signals remain identical when every future bar is removed',()=>{
  const checked=inspectNativePreparation(fixture(),product),g=checked.groups.find(g=>g.ticker==='MYMM6');
