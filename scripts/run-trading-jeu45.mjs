...
import {runStudy45} from '../trading/lab/jeu45-diagnostic.mjs';
...
const hash=b=>createHash('sha256').update(b).digest('hex'),frozen=await readFile('trading/lab/jeu45-freeze.json'),freeze=JSON.parse(frozen);
...
const prePerformanceCommit=execFileSync('git',['log','-1','--format=%H','--','trading/lab/jeu45-freeze.json'],{encoding:'utf8'}).trim();
assert.equal(hash(execFileSync('git',['show',`${prePerformanceCommit}:trading/lab/jeu45-freeze.json`])),hash(frozen));
...
const src=JSON.parse(await readFile('trading/lab/jeu45-source.json'));
...
const checkpoint=data=>{const record={game:45,prePerformanceCommit,startedAt,updatedAt:new Date().toISOString(),...data};writeFileSync(statusPath,JSON.stringify(record,null,2)+'\n');appendFileSync(resolve(outDir,'progress.jsonl'),JSON.stringify(record)+'\n');};
checkpoint({status:'running',completed:0,total:112});
console.log(JSON.stringify({status:'running',game:45,startedAt,prePerformanceCommit,total:112,outDir}));
try{
 const result=runStudy45(prices,mnq,prior,progress=>checkpoint({status:'running',...progress}));
