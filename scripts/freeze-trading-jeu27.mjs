...
for (const path of ['scripts/freeze-trading-jeu27.mjs', 'scripts/run-trading-jeu27.mjs',
  'scripts/test-trading-fresh-reentry.mjs', 'trading/lab/JEU27_PROTOCOL.md', 'trading/lab/jeu27-source.json']) await visit(path);
const freeze = { schema: 'jeu27-freeze-v1', createdAt: new Date().toISOString(),
  beforePerformance: true, files: Object.fromEntries([...files].sort(([a], [b]) => a.localeCompare(b))) };
const raw = JSON.stringify(freeze, null, 2) + '\n';
await writeFile(resolve(root, 'trading/lab/jeu27-freeze.json'), raw, { flag: 'wx' });
