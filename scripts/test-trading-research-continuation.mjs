...
test('new reports preserve all eight failed configurations and their frozen protocols',async()=>{
  ...
  assert.equal(ledger.configurationCount,65);assert.equal(ledger.entries.length,65);assert.equal(ledger.independentConfirmations,0);assert.equal(new Set(ledger.entries.map(x=>x.executionKey)).size,65);
