...
test('new reports preserve all eight failed configurations and their frozen protocols',async()=>{
  ...
  assert.equal(ledger.configurationCount,61);assert.equal(ledger.entries.length,61);assert.equal(ledger.independentConfirmations,0);assert.equal(new Set(ledger.entries.map(x=>x.executionKey)).size,61);
  assert.equal(ledger.entries.filter(x=>x.game<=26).length,57);
