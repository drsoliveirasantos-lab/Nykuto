...
import { combinedContexts, filterCombinedSignals } from '../trading/lab/jeu24-context.mjs';
import { JEU24_POLICY as P, JEU24_PRODUCTS as JEU19_PRODUCTS, JEU24_SCENARIOS as JEU19_SCENARIOS, CONFLUENCE_CHECKS } from '../trading/lab/jeu24-policy.mjs';
...
const freezeRaw=await readFile(resolve(root,'trading/lab/jeu24-freeze.json')), freeze=JSON.parse(freezeRaw);
...
const source=JSON.parse(await readFile(resolve(root,'trading/lab/jeu24-source.json'))), raw=await readFile(resolve(sourceDir,'dataset.json'));
...
if(phase==='train')for(const filename of ['selection.json','report.json','train-runs-private.json','train-context-private.json']){
...
const audit={signalPrefixes:0,accountPrefixes:0,trades:0,referencePairs:0,riskReferencePairs:0,firstSignalPairs:0,contextPrefixes:0,passed:false};
const cache=new Map();
function inputs(market,scenario,end){
  ...
  const combined=filterCombinedSignals(candles,signals,market.product);
...
function run(market,scenario,period,factor,account){
  const {candles,signals,combined}=inputs(market,scenario,period.end), value=simulateConfluence(candles,signals,scenario,market.product,period,factor,account);
  const consumed=new Set();
  assert.ok(value.days.every(d=>d.trades<=2));
  for(const t of value.trades){
    assert.equal(combined.decisions.get(t.signalClose)?.accepted,true,'Executed trade without full confluence');
  ...
  if(account)for(const day of value.days){
    ...
    const prefix=simulateConfluence(candles.filter(b=>b.day<=day.day),signals,scenario,market.product,{...period,end},factor,true);
  ...
  if(phase==='train'){
    const refs=reference.runs.filter(x=>matches(x)&&x.scenario==='admission-risk'+scenario.riskPerTrade);assert.equal(refs.length,1,'Exactly one archived baseline required');
    output.referenceComparison=compareExecutions(refs[0].run,value);audit.referencePairs++;
    if(scenario.riskPerTrade!==50){
      const riskRefs=privateRuns.filter(x=>matches(x)&&x.scenario==='confluence-risk50');assert.equal(riskRefs.length,1,'50 dollar reference must precede other profiles');
      output.riskComparison=compareExecutions(riskRefs[0].run,value);audit.riskReferencePairs++;
    }
  }
...
if(phase==='train'){
  ...
  const selection={schema:'jeu24-selection-v1',id:candidates[0]?.id??null,attempted:results.length,cumulativeAttempts:P.priorAttempts+results.length,freezeSha256:hash(freezeRaw),sourceSha256:source.sha256,trainEnd:P.trainEnd,createdAt:new Date().toISOString()};
  const selectionRaw=serialize(selection);await writeFile(resolve(outDir,'selection.json'),selectionRaw);
  report={schema:'jeu24-report-v1',protocol:P.version,generatedAt:new Date().toISOString(),freezeSha256:hash(freezeRaw),protocolSha256:freeze.files['trading/lab/JEU24_PROTOCOL.md'],source,policy:P,results,selection,selectionSha256:hash(selectionRaw),holdout:{status:selection.id?'pending-frozen-selection':'not-opened',reason:selection.id?'Candidate selected on development only':'No eligible development candidate'},
...
const privateContexts=contextMarkets.map(m=>({symbol:m.product.symbol,period:contextPeriod,decisions:[...inputs(m,JEU19_SCENARIOS[0],contextPeriod.end).combined.decisions].filter(([,d])=>d.day>=contextPeriod.start&&d.day<contextPeriod.end)}));
const confluence=privateContexts.map(m=>{
  const ds=m.decisions.map(([,d])=>d),passes=Object.fromEntries(CONFLUENCE_CHECKS.map(k=>[k,ds.filter(d=>d.checks[k]).length]));let remaining=ds;
  const funnel=CONFLUENCE_CHECKS.map(id=>{remaining=remaining.filter(d=>d.checks[id]);return {id,remaining:remaining.length};});
  return {symbol:m.symbol,period:m.period,signals:ds.length,accepted:ds.filter(d=>d.accepted).length,missingContext:ds.filter(d=>d.missingContext).length,passes,funnel};
});
report.confluence=confluence;
audit.passed=true; report.audit=audit;
await writeFile(resolve(outDir,phase+'-context-private.json'),serialize({schema:'jeu24-context-private-v1',freezeSha256:hash(freezeRaw),markets:privateContexts}));
await writeFile(resolve(outDir,'report.json'),serialize(report));
await writeFile(resolve(outDir,phase+'-runs-private.json'),serialize({schema:'jeu24-private-v1',source,freezeSha256:hash(freezeRaw),runs:privateRuns}));
