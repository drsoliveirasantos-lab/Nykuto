...
import { gradeSignals } from '../trading/lab/jeu25-risk.mjs';
import { JEU25_POLICY as P, JEU25_PRODUCTS as JEU19_PRODUCTS, JEU25_SCENARIOS as JEU19_SCENARIOS } from '../trading/lab/jeu25-policy.mjs';
...
const freezeRaw=await readFile(resolve(root,'trading/lab/jeu25-freeze.json')), freeze=JSON.parse(freezeRaw);
...
const source=JSON.parse(await readFile(resolve(root,'trading/lab/jeu25-source.json'))), raw=await readFile(resolve(sourceDir,'dataset.json'));
...
const strictRaw=await readFile(resolve(sourceDir,'strict-reference-train.json'));assert.equal(strictRaw.length,source.strictReference.bytes);assert.equal(hash(strictRaw),source.strictReference.sha256);
const strictReference=JSON.parse(strictRaw);assert.equal(strictReference.schema,'jeu24-private-v1');
if(phase==='train')for(const filename of ['selection.json','report.json','train-runs-private.json','train-grades-private.json']){
...
const clean=r=>{
  ...
  return {status:r.status,terminalDay:r.terminalDay,net:r.net,balance:r.balance,drawdown:r.drawdown,daily:r.daily,trades:r.trades.length,metrics:m,grossDollars:cents(r.net+feesDollars),feesDollars,pfDollars:losses?wins/losses:null,signalCount:r.signalCount,denied:r.denied,ambiguous:r.ambiguous,recoveredAfterRefusal:r.trades.filter(t=>t.priorRefusals>0).length,tiers:P.riskCaps.map(cap=>{const ts=r.trades.filter(t=>t.approvedRiskCap===cap);return {cap,trades:ts.length,net:cents(ts.reduce((s,t)=>s+t.netDollars,0)),averagePlannedRisk:ts.length?cents(ts.reduce((s,t)=>s+t.riskDollars+t.costDollars,0)/ts.length):null};})};
};
const audit={signalPrefixes:0,accountPrefixes:0,trades:0,referencePairs:0,strictReferencePairs:0,firstSignalPairs:0,contextPrefixes:0,passed:false};
const cache=new Map();
function inputs(market,scenario,end){
  ...
  for(const g of market.data.groups){
    ...
    const bars=g.candles.filter(b=>b.day<end), set=admissionSignals(bars,market.product);
    const first=new Map();for(const [t,s] of set)if(!first.has(s.day+'/'+s.side))first.set(s.day+'/'+s.side,[t,s]);
    assert.deepEqual([...first.values()],[...openingSignals(bars,market.product)]);audit.firstSignalPairs++;
    for(let cut=1;cut<=bars.length;cut++){
      const last=bars[cut-1]; if(last.day<g.start)continue;
      assert.deepEqual([...admissionSignals(bars.slice(0,cut),market.product)],[...set].filter(([t])=>t<=last.time+300)); audit.signalPrefixes++;
    }
  ...
  const combined=gradeSignals(candles,signals,market.product);
  for(const time of signals.keys()){
    const prefix=combinedContexts(candles.filter(b=>b.time+300<=time),market.product);
    assert.deepEqual(prefix.get(time),combined.contexts.get(time),'Future-dependent context');audit.contextPrefixes++;
  }
  const value={candles,signals,combined};cache.set(key,value);return value;
}
const privateRuns=[];
function run(market,scenario,period,factor,account){
  const {candles,signals,combined}=inputs(market,scenario,period.end), value=simulateGraded(candles,signals,scenario,market.product,period,factor,account);
  const consumed=new Set();
  assert.ok(value.days.every(d=>d.trades<=2));
  for(const t of value.trades){
    const grade=combined.grades.get(t.signalClose);assert.equal(t.approvedRiskCap,grade.cap);assert.equal(t.contextScore,grade.score);assert.ok(t.riskDollars+t.costDollars<=grade.cap+1e-8,'Assigned risk cap bypassed');
    ...
    assert.ok(t.riskDollars+t.costDollars<=scenario.riskPerTrade+1e-8);assert.equal(cents(t.stop),cents(t.stopPrice));assert.ok(t.rangeClosedAt<=t.breakoutAt&&t.breakoutAt<=t.signalOpen);
  ...
  if(account)for(const day of value.days){
    ...
    const prefix=simulateGraded(candles.filter(b=>b.day<=day.day),signals,scenario,market.product,{...period,end},factor,true);
  ...
  if(phase==='train'){
    ...
    const strictRefs=strictReference.runs.filter(x=>matches(x)&&x.scenario==='confluence-risk150');assert.equal(strictRefs.length,1,'Exactly one archived strict reference required');
    output.strictComparison=compareExecutions(strictRefs[0].run,value);output.strictDrawdown=strictRefs[0].run.drawdown;audit.strictReferencePairs++;
...
if(phase==='train'){
  ...
  const selection={schema:'jeu25-selection-v1',id:candidates[0]?.id??null,attempted:results.length,cumulativeAttempts:P.priorAttempts+results.length,freezeSha256:hash(freezeRaw),sourceSha256:source.sha256,trainEnd:P.trainEnd,createdAt:new Date().toISOString()};
  const selectionRaw=serialize(selection);await writeFile(resolve(outDir,'selection.json'),selectionRaw);
  report={schema:'jeu25-report-v1',protocol:P.version,generatedAt:new Date().toISOString(),freezeSha256:hash(freezeRaw),protocolSha256:freeze.files['trading/lab/JEU25_PROTOCOL.md'],source,policy:P,results,selection,selectionSha256:hash(selectionRaw),holdout:{status:selection.id?'pending-frozen-selection':'not-opened',reason:selection.id?'Candidate selected on development only':'No eligible development candidate'},
...
const contextPeriod=phase==='train'?{start:P.from,end:P.trainEnd}:{start:P.trainEnd,end:P.end};
const contextMarkets=phase==='train'?markets:markets.filter(m=>report.selection.id?.startsWith(m.product.symbol+'/'));
const privateGrades=contextMarkets.map(m=>({symbol:m.product.symbol,period:contextPeriod,decisions:[...inputs(m,JEU19_SCENARIOS[0],contextPeriod.end).combined.grades].filter(([,d])=>d.day>=contextPeriod.start&&d.day<contextPeriod.end)}));
report.grading=privateGrades.map(m=>{
  const ds=m.decisions.map(([,d])=>d);return {symbol:m.symbol,period:m.period,signals:ds.length,missingContext:ds.filter(d=>d.missingContext).length,tiers:P.riskCaps.map(cap=>({cap,signals:ds.filter(d=>d.cap===cap).length})),scores:Array.from({length:6},(_,score)=>({score,signals:ds.filter(d=>d.score===score).length}))};
});
audit.passed=true; report.audit=audit;
await writeFile(resolve(outDir,phase+'-grades-private.json'),serialize({schema:'jeu25-grades-private-v1',freezeSha256:hash(freezeRaw),markets:privateGrades}));
await writeFile(resolve(outDir,'report.json'),serialize(report));
await writeFile(resolve(outDir,phase+'-runs-private.json'),serialize({schema:'jeu25-private-v1',source,freezeSha256:hash(freezeRaw),runs:privateRuns}));
