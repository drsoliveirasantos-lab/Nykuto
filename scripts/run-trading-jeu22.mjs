...
import { JEU22_POLICY as P, JEU22_PRODUCTS as JEU19_PRODUCTS, JEU22_SCENARIOS as JEU19_SCENARIOS } from '../trading/lab/jeu22-policy.mjs';
...
const freezeRaw=await readFile(resolve(root,'trading/lab/jeu22-freeze.json')), freeze=JSON.parse(freezeRaw);
...
const source=JSON.parse(await readFile(resolve(root,'trading/lab/jeu22-source.json'))), raw=await readFile(resolve(sourceDir,'dataset.json'));
...
function inputs(market,scenario,end){
  ...
  for(const g of market.data.groups){
    ...
    const bars=g.candles.filter(b=>b.day<end), set=openingSignals(bars,market.product);
    for(let cut=1;cut<=bars.length;cut++){
      const last=bars[cut-1]; if(last.day<g.start)continue;
      assert.deepEqual([...openingSignals(bars.slice(0,cut),market.product)],[...set].filter(([t])=>t<=last.time+300)); audit.signalPrefixes++;
    }
...
function run(market,scenario,period,factor,account){
  const {candles,signals}=inputs(market,scenario,period.end), value=simulateOpening(candles,signals,scenario,market.product,period,factor,account);
  for(const t of value.trades){
    assert.ok(t.signalClose===t.entryTime&&t.signalOpen<t.entryTime&&t.trendClosedAt<=t.entryTime);
    assert.ok(t.riskDollars+t.costDollars<=50+1e-8);assert.equal(cents(t.stop),cents(t.stopPrice));assert.ok(t.rangeClosedAt<=t.breakoutAt&&t.breakoutAt<=t.signalOpen);
  ...
  if(account)for(const day of value.days){
    ...
    const prefix=simulateOpening(candles.filter(b=>b.day<=day.day),signals,scenario,market.product,{...period,end},factor,true);
...
if(phase==='train'){
  ...
  const selection={schema:'jeu22-selection-v1',id:candidates[0]?.id??null,attempted:results.length,cumulativeAttempts:P.priorAttempts+results.length,freezeSha256:hash(freezeRaw),sourceSha256:source.sha256,trainEnd:P.trainEnd,createdAt:new Date().toISOString()};
  const selectionRaw=serialize(selection);await writeFile(resolve(outDir,'selection.json'),selectionRaw);
  report={schema:'jeu22-report-v1',protocol:P.version,generatedAt:new Date().toISOString(),freezeSha256:hash(freezeRaw),protocolSha256:freeze.files['trading/lab/JEU22_PROTOCOL.md'],source,policy:P,results,selection,selectionSha256:hash(selectionRaw),holdout:{status:selection.id?'pending-frozen-selection':'not-opened',reason:selection.id?'Candidate selected on development only':'No eligible development candidate'},
...
await writeFile(resolve(outDir,phase+'-runs-private.json'),serialize({schema:'jeu22-private-v1',source,freezeSha256:hash(freezeRaw),runs:privateRuns}));
