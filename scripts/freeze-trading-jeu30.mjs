...
for (const path of ['scripts/freeze-trading-jeu30.mjs', 'scripts/run-trading-jeu30.mjs',
  'scripts/test-trading-august.mjs', 'trading/lab/JEU30_PROTOCOL.md', 'trading/lab/jeu30-source.json']) await visit(path);
const freeze = { schema: 'jeu30-freeze-v1', createdAt: new Date().toISOString(),
  beforePerformance: true, files: Object.fromEntries([...files].sort(([a], [b]) => a.localeCompare(b))) };
const raw = JSON.stringify(freeze, null, 2) + '\n';
await writeFile(resolve(root, 'trading/lab/jeu30-freeze.json'), raw, { flag: 'wx' });
