...
import { JEU23_POLICY as P, JEU23_PRODUCTS as JEU19_PRODUCTS, JEU23_SCENARIOS as JEU19_SCENARIOS } from '../trading/lab/jeu23-policy.mjs';
...
const freezeRaw=await readFile(resolve(root,'trading/lab/jeu23-freeze.json')), freeze=JSON.parse(freezeRaw);
...
const source=JSON.parse(await readFile(resolve(root,'trading/lab/jeu23-source.json'))), raw=await readFile(resolve(sourceDir,'dataset.json'));
...
const reference=JSON.parse(referenceRaw);assert.equal(reference.schema,'jeu22-private-v1');
...
const audit={signalPrefixes:0,accountPrefixes:0,trades:0,referencePairs:0,riskReferencePairs:0,firstSignalPairs:0,passed:false};
const cache=new Map();
...
function run(market,scenario,period,factor,account){
  const {candles,signals}=inputs(market,scenario,period.end), value=simulateAdmission(candles,signals,scenario,market.product,period,factor,account);
  ...
  if(account)for(const day of value.days){
    ...
    const prefix=simulateAdmission(candles.filter(b=>b.day<=day.day),signals,scenario,market.product,{...period,end},factor,true);
  ...
  if(phase==='train'){
    const refs=reference.runs.filter(matches);assert.equal(refs.length,1,'Exactly one archived baseline required');
    output.referenceComparison=compareExecutions(refs[0].run,value);audit.referencePairs++;
    if(scenario.riskPerTrade!==50){
      const riskRefs=privateRuns.filter(x=>matches(x)&&x.scenario==='admission-risk50');assert.equal(riskRefs.length,1,'50 dollar reference must precede other profiles');
...
if(phase==='train'){
  ...
  const selection={schema:'jeu23-selection-v1',id:candidates[0]?.id??null,attempted:results.length,cumulativeAttempts:P.priorAttempts+results.length,freezeSha256:hash(freezeRaw),sourceSha256:source.sha256,trainEnd:P.trainEnd,createdAt:new Date().toISOString()};
  const selectionRaw=serialize(selection);await writeFile(resolve(outDir,'selection.json'),selectionRaw);
  report={schema:'jeu23-report-v1',protocol:P.version,generatedAt:new Date().toISOString(),freezeSha256:hash(freezeRaw),protocolSha256:freeze.files['trading/lab/JEU23_PROTOCOL.md'],source,policy:P,results,selection,selectionSha256:hash(selectionRaw),holdout:{status:selection.id?'pending-frozen-selection':'not-opened',reason:selection.id?'Candidate selected on development only':'No eligible development candidate'},
...
await writeFile(resolve(outDir,phase+'-runs-private.json'),serialize({schema:'jeu23-private-v1',source,freezeSha256:hash(freezeRaw),runs:privateRuns}));
