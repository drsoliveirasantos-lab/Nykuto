...
import { pivotContexts, structuralSignals } from '../trading/lab/jeu16-engine.mjs';
import { ablationTerms, simulateAblation } from '../trading/lab/jeu17-engine.mjs';
import { JEU17_POLICY as policy, JEU17_SCENARIOS as scenarios, JEU17_CONTRASTS as contrasts } from '../trading/lab/jeu17-policy.mjs';
...
const freezeText = await readFile(resolve(root, 'trading/lab/jeu17-freeze.json'), 'utf8'), freeze = JSON.parse(freezeText);
...
assert.equal(hash(oldRaw), 'fca25015dedc4fd8e921bee121c860b9faa2b58498a7661b5afd9f2b37e56fc3');
...
function auditPivot(t, scenario) {
  ...
  assert.equal(t.stop, p.price - sign * .25); if (scenario.netMargin) assert.ok(t.expectedNetRR >= 1);
  if (scenario.netMargin) assert.ok(t.targetDistance * 2 - t.costDollars >= t.riskDollars + t.costDollars);
...
function auditRun(run, scenario, account, factor) {
  ...
  for (const day of run.days) {
    ...
    for (const t of ts) {
      ...
      if (scenario.netMargin) assert.ok(t.targetDistance * 2 - t.costDollars >= t.riskDollars + t.costDollars);
      if (scenario.stopMode === 'pivot') { auditPivot(t, scenario); const x = ablationTerms({ ...t, pivot: t.pivot }, t.entry, t.entryTime, scenario, factor); assert.equal(x.terms.risk, t.risk); }
...
for (const scenario of scenarios) {
  const signals = maps[scenario.stopMode], diagnostic = [1, 2].map(f => simulateAblation(candles, signals, scenario, bounds, f, false));
  for (const [i, run] of diagnostic.entries()) auditRun(run, scenario, false, i + 1);
  if (['atr5', 'pivot5'].includes(scenario.id)) {
    const old = oldRuns.find(x => x.scenario === scenario.id && x.diagnostic).diagnostic;
  ...
  for (const w of JEU14_WINDOWS) {
    ...
    for (const factor of [1, 2]) {
      const run = simulateAblation(bars, signals, scenario, w, factor, true); auditRun(run, scenario, true, factor);
      if (['atr5', 'pivot5'].includes(scenario.id)) {
        const old = oldRuns.find(x => x.scenario === scenario.id && x.window?.start === w.start)[factor === 1 ? 'normal' : 'stress'];
        ...
        if (scenario.id === 'atr5') assert.deepEqual(run.trades, simulateLucid(bars, signals, { guarded: true }, w, factor, true).trades);
      }
      for (let cut = 1; cut <= bars.length; cut++) {
        const last = bars[cut - 1]; if (last.minute !== last.closeMinute - 5) continue;
        const prefix = simulateAblation(bars.slice(0, cut), signals, scenario, w, factor, true);
...
const effects = contrasts.map(c => ({ ...c, ...Object.fromEntries(['normal', 'stress'].map(factor => {
  const from = results.find(r => r.id === c.from).diagnostic[factor], to = results.find(r => r.id === c.to).diagnostic[factor];
  return [factor, { netDollars: to.net - from.net, grossDollars: to.grossDollars - from.grossDollars, feesDollars: to.feesDollars - from.feesDollars, trades: to.trades - from.trades }];
})) }));
audit.passed = true;
const report = { schema: 'jeu17-report-v1', protocol: policy.version, generatedAt: new Date().toISOString(), freezeSha256: hash(freezeText), protocolSha256: freeze.files['trading/lab/JEU17_PROTOCOL.md'], source, policy, riskPolicy,
  coverage: { expectedSessions, scoredSessions: eligible.length, first: eligible[0].date, last: eligible.at(-1).date, unavailable: unavailable.length, completeWindows: 5, blockedWindows: 3 }, results, effects, audit,
...
await writeFile(resolve(outDir, 'runs-private.json'), serialize({ schema: 'jeu17-private-v1', freezeSha256: report.freezeSha256, source, runs: privateRuns }));
