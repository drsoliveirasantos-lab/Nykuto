...
for (const path of ['scripts/freeze-trading-jeu29.mjs', 'scripts/run-trading-jeu29.mjs',
  'scripts/test-trading-portfolio.mjs', 'trading/lab/JEU29_PROTOCOL.md', 'trading/lab/jeu29-source.json']) await visit(path);
const freeze = { schema: 'jeu29-freeze-v1', createdAt: new Date().toISOString(),
  beforePerformance: true, files: Object.fromEntries([...files].sort(([a], [b]) => a.localeCompare(b))) };
const raw = JSON.stringify(freeze, null, 2) + '\n';
await writeFile(resolve(root, 'trading/lab/jeu29-freeze.json'), raw, { flag: 'wx' });
