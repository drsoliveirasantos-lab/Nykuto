...
import { JEU28_POLICY as P, JEU28_PRODUCTS as JEU19_PRODUCTS, JEU28_SCENARIOS as JEU19_SCENARIOS } from '../trading/lab/jeu28-policy.mjs';
...
const freezeRaw=await readFile(resolve(root,'trading/lab/jeu28-freeze.json')), freeze=JSON.parse(freezeRaw);
...
const source=JSON.parse(await readFile(resolve(root,'trading/lab/jeu28-source.json'))), raw=await readFile(resolve(sourceDir,'dataset.json'));
...
const clean=r=>{
  ...
  return {status:r.status,terminalDay:r.terminalDay,net:r.net,balance:r.balance,drawdown:r.drawdown,daily:r.daily,trades:r.trades.length,metrics:m,grossDollars:cents(r.net+feesDollars),feesDollars,pfDollars:losses?wins/losses:null,signalCount:r.signalCount,denied:r.denied,ambiguous:r.ambiguous,recoveredAfterRefusal:r.trades.filter(t=>t.priorRefusals>0).length,breakEvenArmed:r.trades.filter(t=>t.breakEvenAt!==null).length,breakEvenExits:r.trades.filter(t=>t.reason.startsWith('Break-even')).length,breakEvenExitNet:cents(r.trades.filter(t=>t.reason.startsWith('Break-even')).reduce((n,t)=>n+t.netDollars,0)),dailyMean:cents(r.net/r.days.length),activeDailyMean:r.days.some(d=>d.trades)?cents(r.net/r.days.filter(d=>d.trades).length):null};
};
...
function run(market,scenario,period,factor,account){
  const {candles,signals}=inputs(market,scenario,period.end), value=simulateBreakEven(candles,signals,scenario,market.product,period,factor,account);
  const consumed=new Set();
  assert.ok(value.days.every(d=>d.trades<=2));
  for(const t of value.trades){
    assert.ok(!consumed.has(t.day+'/'+t.side));
    if(t.breakEvenAt!==null){assert.ok(t.breakEvenAt>=t.entryTime+300&&t.breakEvenAt<=t.exitTime);assert.ok((t.side==='Long'?1:-1)*(t.stop-t.initialStop)>0);const protectedNet=(t.side==='Long'?1:-1)*(t.stop-t.entry)*market.product.multiplier-t.costDollars;assert.ok(protectedNet>=-1e-7&&protectedNet<market.product.tick*market.product.multiplier+1e-7);}else assert.equal(t.stop,t.initialStop);
    consumed.add(t.day+'/'+t.side);
    assert.ok(t.signalClose===t.entryTime&&t.signalOpen<t.entryTime&&t.trendClosedAt<=t.entryTime);
    assert.ok(t.riskDollars+t.costDollars<=scenario.riskPerTrade+1e-8);assert.equal(cents(t.initialStop),cents(t.stopPrice));assert.ok(t.side==='Long'?t.entry>t.rangeHigh:t.entry<t.rangeLow);assert.ok(t.rangeClosedAt<=t.breakoutAt&&t.breakoutAt<=t.signalOpen);
  ...
  if(account)for(const day of value.days){
    ...
    const prefix=simulateBreakEven(candles.filter(b=>b.day<=day.day),signals,scenario,market.product,{...period,end},factor,true);
  ...
  if(phase==='train'){
    ...
    output.referenceComparison=compareExecutions(refs[0].run,value);output.protectionComparison=compareProtection(refs[0].run,value);output.referenceDrawdown=refs[0].run.drawdown;audit.referencePairs++;
...
if(phase==='train'){
  ...
  const selection={schema:'jeu28-selection-v1',id:candidates[0]?.id??null,attempted:results.length,cumulativeAttempts:P.priorAttempts+results.length,freezeSha256:hash(freezeRaw),sourceSha256:source.sha256,trainEnd:P.trainEnd,createdAt:new Date().toISOString()};
  const selectionRaw=serialize(selection);await writeFile(resolve(outDir,'selection.json'),selectionRaw);
  report={schema:'jeu28-report-v1',protocol:P.version,generatedAt:new Date().toISOString(),freezeSha256:hash(freezeRaw),protocolSha256:freeze.files['trading/lab/JEU28_PROTOCOL.md'],source,policy:P,results,selection,selectionSha256:hash(selectionRaw),holdout:{status:selection.id?'pending-frozen-selection':'not-opened',reason:selection.id?'Candidate selected on development only':'No eligible development candidate'},
...
await writeFile(resolve(outDir,phase+'-runs-private.json'),serialize({schema:'jeu28-private-v1',source,freezeSha256:hash(freezeRaw),runs:privateRuns}));
