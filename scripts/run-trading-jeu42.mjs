...
import {runStudy42} from '../trading/lab/jeu42-diagnostic.mjs';
...
const hash=b=>createHash('sha256').update(b).digest('hex'),frozen=await readFile('trading/lab/jeu42-freeze.json'),freeze=JSON.parse(frozen);
...
const prePerformanceCommit=execFileSync('git',['log','-1','--format=%H','--','trading/lab/jeu42-freeze.json'],{encoding:'utf8'}).trim();
assert.equal(hash(execFileSync('git',['show',`${prePerformanceCommit}:trading/lab/jeu42-freeze.json`])),hash(frozen));
...
const src=JSON.parse(await readFile('trading/lab/jeu42-source.json'));
...
await mkdir(outDir);
const result=await runStudy42(prices,mnq,prior);
