...
import { JEU21_POLICY as P, JEU21_PRODUCTS as JEU19_PRODUCTS, JEU21_SCENARIOS as JEU19_SCENARIOS } from '../trading/lab/jeu21-policy.mjs';
...
const freezeRaw=await readFile(resolve(root,'trading/lab/jeu21-freeze.json')), freeze=JSON.parse(freezeRaw);
...
const source=JSON.parse(await readFile(resolve(root,'trading/lab/jeu21-source.json'))), raw=await readFile(resolve(sourceDir,'dataset.json'));
...
if(phase==='train'){
  ...
  const selection={schema:'jeu21-selection-v1',id:candidates[0]?.id??null,attempted:results.length,cumulativeAttempts:P.priorAttempts+results.length,freezeSha256:hash(freezeRaw),sourceSha256:source.sha256,trainEnd:P.trainEnd,createdAt:new Date().toISOString()};
  const selectionRaw=serialize(selection);await writeFile(resolve(outDir,'selection.json'),selectionRaw);
  report={schema:'jeu21-report-v1',protocol:P.version,generatedAt:new Date().toISOString(),freezeSha256:hash(freezeRaw),protocolSha256:freeze.files['trading/lab/JEU21_PROTOCOL.md'],source,policy:P,results,selection,selectionSha256:hash(selectionRaw),holdout:{status:selection.id?'pending-frozen-selection':'not-opened',reason:selection.id?'Candidate selected on development only':'No eligible development candidate'},
...
await writeFile(resolve(outDir,phase+'-runs-private.json'),serialize({schema:'jeu21-private-v1',source,freezeSha256:hash(freezeRaw),runs:privateRuns}));
