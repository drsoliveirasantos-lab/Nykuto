...
import { JEU14_SOURCE } from '../trading/lab/jeu14-source.mjs';
...
import { JEU19_POLICY as P, JEU19_PRODUCTS, JEU19_SCENARIOS, JEU19_WINDOWS } from '../trading/lab/jeu19-policy.mjs';
...
const freezeRaw=await readFile(resolve(root,'trading/lab/jeu19-freeze.json')), freeze=JSON.parse(freezeRaw);
...
const source=JSON.parse(await readFile(resolve(root,'trading/lab/jeu19-source.json'))), raw=await readFile(resolve(sourceDir,'dataset.json'));
assert.equal(raw.length,source.bytes); assert.equal(hash(raw),source.sha256);
const bundle=JSON.parse(raw), mnqRaw=await readFile(resolve(sourceDir,'mnq-dataset.json'));
assert.equal(mnqRaw.length,JEU14_SOURCE.bytes); assert.equal(hash(mnqRaw),JEU14_SOURCE.sha256);
const mnq=inspectHistory(JSON.parse(mnqRaw));
const markets=JEU19_PRODUCTS.map(product=>({product, data:product.symbol==='MNQ' ? mnq : inspectMultimarket(bundle.products.find(p=>p.symbol===product.symbol),product)}));
...
function inputs(market,scenario,end){
  ...
  for(const g of market.data.groups){
    ...
    const bars=g.candles.filter(b=>b.day<end), set=multimarketSignals(bars,scenario.id);
    for(let cut=6;cut<=bars.length;cut+=6){
      const last=bars[cut-1]; if(last.minute!==last.closeMinute-5||last.day<g.start)continue;
      assert.deepEqual([...multimarketSignals(bars.slice(0,cut),scenario.id)],[...set].filter(([t])=>t<=last.time+300)); audit.signalPrefixes++;
    }
...
if(phase==='train'){
  ...
  const selection={schema:'jeu19-selection-v1',id:candidates[0]?.id??null,attempted:results.length,freezeSha256:hash(freezeRaw),sourceSha256:source.sha256,trainEnd:P.trainEnd,createdAt:new Date().toISOString()};
  const selectionRaw=serialize(selection);await writeFile(resolve(outDir,'selection.json'),selectionRaw);
  report={schema:'jeu19-report-v1',protocol:P.version,generatedAt:new Date().toISOString(),freezeSha256:hash(freezeRaw),protocolSha256:freeze.files['trading/lab/JEU19_PROTOCOL.md'],source,policy:P,results,selection,selectionSha256:hash(selectionRaw),holdout:{status:selection.id?'pending-frozen-selection':'not-opened',reason:selection.id?'Candidate selected on development only':'No eligible development candidate'},
...
await writeFile(resolve(outDir,phase+'-runs-private.json'),serialize({schema:'jeu19-private-v1',source,freezeSha256:hash(freezeRaw),runs:privateRuns}));
