...
test('shared history keeps all 266 monthly cost views exact, includes ten games and excludes the continuous summer account',()=>{
 let count=0;
 for(const g of manifest.games){
  ...
  for(const variant of model.variants)for(const mode of model.modes)for(const cost of ['normal','stress']){
   const selection=selectHistory(model,{variant:variant.id,mode,cost,day:'2026-07-14'});assert.equal(selection.months.length,['40','41','42'].includes(g.id)?8:3);
   for(const month of selection.months){count++;const v=source.views.find(v=>(v.month??v.id)===month.month&&(v.variant??v.profileId)===variant.id&&(v.mode??(g.id==='33'?'evaluation':'funded'))===mode),c=v.costs[cost],d=month.result.calendar.daily;
    ...
    if(['39','40','41','42'].includes(g.id))assert.deepEqual(d.map(r=>[r.markets,r.averageRiskUSD]),c.calendar.daily.map(r=>[r.markets,r.averageRiskUSD]));
    assert.equal(month.period.start,v.period.start);assert.equal(month.period.end,v.period.end);
   }
  }
 }
 assert.equal(count,266);assert.equal(manifest.games.length,10);assert.equal(models.get('40').views.length,8);assert.equal(selectHistory(models.get('40')).variant,'fixed100');assert.equal(models.get('39').views.length,9);assert.equal(selectHistory(models.get('39')).variant,'mnq-kronos');assert.equal(models.get('33').views.length,12);assert.ok(models.get('33').views.every(v=>v.month!=='summer'));
...
test('all calendar layouts retain exact date positions, non-studied dates, stopped nulls and truthful optional fields',()=>{
 for(const model of models.values())for(const variant of model.variants)for(const mode of model.modes)for(const cost of ['normal','stress']){
  ...
  const eight=['40','41','42'].includes(model.game.id);assert.equal(all.length,eight?243:92);assert.equal(all.filter(d=>d.state==='weekend').length,eight?70:26);assert.deepEqual(all.filter(d=>d.state==='not-studied').map(d=>d.day),eight?['2026-01-01','2026-01-19','2026-02-16','2026-04-03','2026-05-25','2026-06-19','2026-07-03']:['2026-06-19','2026-07-03']);
...
test('all 72 selectable histories render their own month ranges, retain selected dates and persist only display choices',async()=>{
 const {el,tiles,controller,storage}=await fixture();assert.equal(el('historyCalendarResults').hidden,false);
 assert.equal(controller.getSelection().game,'42');assert.equal(controller.getSelection().variant,'mes-pullback-volume');assert.equal(controller.getSelection().day,'2026-01-01');
 ...
 for(const g of manifest.games){
  ...
  for(const variant of model.variants)for(const mode of model.modes)for(const cost of ['normal','stress']){
   ...
   const selection=controller.getSelection(),d=historyDay(selection);assert.equal(selection.game,g.id);assert.equal(selection.day,'2026-07-14');assert.equal(el('historyCalendarCalendars').children.length,['40','41','42'].includes(g.id)?8:3);assert.equal(tiles().length,['40','41','42'].includes(g.id)?243:92);
