...
import { JEU27_POLICY as P, JEU27_PRODUCTS as JEU19_PRODUCTS, JEU27_SCENARIOS as JEU19_SCENARIOS } from '../trading/lab/jeu27-policy.mjs';
...
const source=JSON.parse(await readFile(resolve(root,'trading/lab/jeu27-source.json'))), raw=await readFile(resolve(sourceDir,'dataset.json'));
...
function run(market,scenario,period,factor,account){
  const {candles,signals}=inputs(market,scenario,period.end), value=simulateReentry(candles,signals,scenario,market.product,period,factor,account);
  const consumed=new Set();
  assert.ok(value.days.every(d=>d.trades<=2));
  for(const t of value.trades){
    assert.ok(t.entryNumber<=P.maxTradesPerDay);
    if(t.repeatedSide){assert.ok(consumed.has(t.day+'/'+t.side));assert.ok(t.breakoutAt-300>=t.previousSameSideExitClose);}
    else assert.ok(!consumed.has(t.day+'/'+t.side));
    consumed.add(t.day+'/'+t.side);
    assert.ok(t.signalClose===t.entryTime&&t.signalOpen<t.entryTime&&t.trendClosedAt<=t.entryTime);
    assert.ok(t.riskDollars+t.costDollars<=scenario.riskPerTrade+1e-8);assert.equal(cents(t.stop),cents(t.stopPrice));assert.ok(t.side==='Long'?t.entry>t.rangeHigh:t.entry<t.rangeLow);assert.ok(t.rangeClosedAt<=t.breakoutAt&&t.breakoutAt<=t.signalOpen);
  ...
  if(account)for(const day of value.days){
    ...
    const prefix=simulateReentry(candles.filter(b=>b.day<=day.day),signals,scenario,market.product,{...period,end},factor,true);
...
if(phase==='train'){
  ...
  const selection={schema:'jeu27-selection-v1',id:candidates[0]?.id??null,attempted:results.length,cumulativeAttempts:P.priorAttempts+results.length,freezeSha256:hash(freezeRaw),sourceSha256:source.sha256,trainEnd:P.trainEnd,createdAt:new Date().toISOString()};
  const selectionRaw=serialize(selection);await writeFile(resolve(outDir,'selection.json'),selectionRaw);
  report={schema:'jeu27-report-v1',protocol:P.version,generatedAt:new Date().toISOString(),freezeSha256:hash(freezeRaw),protocolSha256:freeze.files['trading/lab/JEU27_PROTOCOL.md'],source,policy:P,results,selection,selectionSha256:hash(selectionRaw),holdout:{status:selection.id?'pending-frozen-selection':'not-opened',reason:selection.id?'Candidate selected on development only':'No eligible development candidate'},
...
await writeFile(resolve(outDir,phase+'-runs-private.json'),serialize({schema:'jeu27-private-v1',source,freezeSha256:hash(freezeRaw),runs:privateRuns}));
