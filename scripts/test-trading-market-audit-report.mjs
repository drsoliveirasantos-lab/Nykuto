...
const rsiRaw = await readFile(new URL('../trading/lab/rsi-zone-report.json', import.meta.url));
const rsiReport = JSON.parse(rsiRaw);
...
test('all 32 market/window/cost views show exact winners, losers, confirmations and refusal counts', async () => {
  ...
  try {
    ...
    globalThis.fetch = async url => new Response(url === './rsi-zone-report.json' ? rsiRaw : raw, { headers: { 'Content-Type': 'application/json' } });
    ...
    for (const v of report.views) for (const cost of ['normal', 'stress']) for (const m of v.costs[cost].markets) {
      ...
      assert.equal(el('RsiZones').children.length, 4); assert.equal(el('RsiEvents').children.length, 4); assert.equal(el('RsiSides').children.length, 8);
      const rm = rsiReport.views.find(x => x.id === v.id && x.mode === v.mode).costs[cost].markets.find(x => x.symbol === m.symbol);
      for (const [i, z] of rm.zones.entries()) { assert.equal(el('RsiZones').children[i].children[1].textContent, String(z.count)); assert.equal(el('RsiZones').children[i].children[5].textContent, money(z.net)); }
...
test('failed audit refresh removes stale conclusions and statistics and preserves selected choices on recovery', async () => {
  ...
  try {
    ...
    Object.defineProperty(el('Retry'), 'disabled', { set(v) { if (!v) ready.resolve(); } });
    globalThis.document = { getElementById(id) { assert.ok(elements.has(id), id); return elements.get(id); }, createElement() { return new Element(); } };
    globalThis.fetch = async url => new Response(good ? url === './rsi-zone-report.json' ? rsiRaw : raw : 'bad', { headers: { 'Content-Type': 'application/json' } });
    ...
    for (const id of ['Overview', 'Outcomes', 'Confirmations', 'Features', 'Groups', 'Refusals', 'CostEffects', 'RsiZones', 'RsiEvents', 'RsiSides']) assert.equal(el(id).children.length, 0);
