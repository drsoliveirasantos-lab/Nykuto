...
import {runStudy44} from '../trading/lab/jeu44-diagnostic.mjs';
...
const hash=b=>createHash('sha256').update(b).digest('hex'),frozen=await readFile('trading/lab/jeu44-freeze.json'),freeze=JSON.parse(frozen);
for(const[path,pin]of Object.entries(freeze.files))assert.equal(hash(await readFile(path)),pin,path);
verifyPolicy44Protocol(await readFile('trading/lab/JEU44_PROTOCOL.md','utf8'));
const prePerformanceCommit=execFileSync('git',['log','-1','--format=%H','--','trading/lab/jeu44-freeze.json'],{encoding:'utf8'}).trim();
assert.equal(hash(execFileSync('git',['show',`${prePerformanceCommit}:trading/lab/jeu44-freeze.json`])),hash(frozen));
...
const src=JSON.parse(await readFile('trading/lab/jeu44-source.json'));
...
const checkpoint=data=>{const record={game:44,prePerformanceCommit,startedAt,updatedAt:new Date().toISOString(),...data};writeFileSync(statusPath,JSON.stringify(record,null,2)+'\n');appendFileSync(resolve(outDir,'progress.jsonl'),JSON.stringify(record)+'\n');};
checkpoint({status:'running',completed:0,total:112});
console.log(JSON.stringify({status:'running',game:44,startedAt,prePerformanceCommit,total:112,outDir}));
try{
 const result=runStudy44(prices,mnq,prior,progress=>checkpoint({status:'running',...progress}));
 ...
 checkpoint({status:'completed',completed:112,total:112,outputs,audit:result.report.audit,selection:null,executionAllowed:false});
