...
## Supplied trading knowledge packs — September 9, 2026

`trading/knowledge/` imports Diego's RSI/trend, price-action and market-news
research as 23 hashed source documents and a reproducible 205-record catalogue.
It contains 99 concept cards, 77 news resources, 28 course sections and one
causality policy. References are namespaced by pack. `knowledge-core.mjs`
retrieves text with provenance, limitations and non-execution guardrails;
`analysis/analysis.mjs` actually consumes the contextual references through
`knowledge/analysis-knowledge.mjs`. `/knowledge/` provides the searchable
reading surface. The existing membership middleware protects both routes.
No LLM call, model training, news connection, numerical detector, economic
strategy change or activation is implied. The flat-RSI and structure-definition
differences are recorded in `trading/knowledge/INTEGRATION_AUDIT.md`.
`catalogue.json` and its SHA pin are generated by
