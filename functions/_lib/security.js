...
export async function credentialDigest(username, password, pepper, salt, iterations) {
  const rounds = Number(iterations);
  if (!salt || !Number.isSafeInteger(rounds) || rounds < 100000 || rounds > 1000000) {
    throw new Error('INVALID_CREDENTIAL_CONFIGURATION');
  }
  const pepperedCredential = await hmac(`credential:v2:${username.trim().toLowerCase()}:${password}`, pepper);
  const key = await crypto.subtle.importKey('raw', encoder.encode(pepperedCredential), 'PBKDF2', false, ['deriveBits']);
  const digest = await crypto.subtle.deriveBits(
    { name: 'PBKDF2', hash: 'SHA-256', salt: base64UrlToBytes(salt), iterations: rounds },
    key,
    256
  );
  return bytesToBase64Url(new Uint8Array(digest));
