// Separate Game32 derivative; all original engines stay frozen. Reference mode
// is verified against every archived August Game31 run before new comparisons.
...
import { JEU32_POLICY as P, JEU29_PROFILES, JEU29_PRODUCTS, JEU29_RISK, variantFor } from './jeu32-policy.mjs';
import { alignmentDecision } from './jeu32-alignment.mjs';
import { sizedTerms, sizedRiskGate } from './jeu32-risk.mjs';
...
export function simulateSummerPortfolio(streams, period, costFactor = 1, account = false, variantId = 'reference31') {
  const variant = variantFor(variantId), policy = variant.custom ? variant : JEU29_RISK;
  ...
  for (let i = 0; i < ordered[0].candles.length; i++) {
    ...
    for (const s of ordered) {
      ...
      if (variant.aligned) { const gate = alignmentDecision(signal, signal.alignmentContext); if (!gate.allowed) { reject(s.symbol, bar.time, gate.reason); continue; } }
      const decision = variant.custom ? sizedTerms(signal, bar.open, bar.time, s.product, costFactor, variantId) : (s.strategy === 'failure' ? failedBreakoutTerms : openingTerms)(signal, bar.open, bar.time, s.product, costFactor);
      if (decision.blocked) { reject(s.symbol, bar.time, decision.blocked); continue; }
      const terms = decision.terms, blocked = variant.custom ? sizedRiskGate({ balance, floor, dayStart, ...terms, account }, variantId) : riskGate({ balance, floor, dayStart, ...terms, account }, JEU29_RISK);
      if (blocked) { reject(s.symbol, bar.time, blocked); continue; }
      const sign = signal.side === 'Long' ? 1 : -1, stop = cents(bar.open - sign * terms.risk);
      if (account && !variant.custom) {
    ...
    if (position) {
      ...
      else if (!variant.custom && s.strategy === 'protection') { const change = closedBreakEven(position, bar, s.product); if (change) Object.assign(position, change); }
    }
  }
