...
Exploratory three-month diagnostic on Diego's merged MNQ 1-minute TradingView exports. This branch adds the V8 context engine but does **not** promote it to live execution.
...
The RSI veto removed four trades while preserving the 12 winners in this reconstruction, improving +2 R to +6 R. This is promising as a **candidate filter**, but July and August remain negative and the sample is only 30 filtered trades. It is therefore not a validated improvement and is not promoted into the verified reference.
...
The main conclusion is negative but useful: RSI exits, divergence and MSS are **not** individually strong enough to become standalone entry commands. Even the combined reversal context is not yet an economically defined strategy because no dedicated stop, target and admission geometry have been fixed.
...
These small session subsets are insufficient for promotion. Session must stay a stratification variable, not a free score point.

## Decision

1. Keep **ORB continuation** and **structural reversal** as separate strategy families.
2. Add the RSI anti-late-entry rule as a **research candidate**, not an active production rule.
...
4. Do not make MSS mandatory for the ORB family: it would change the strategy class and can collapse frequency.
5. Preserve the verified Jeu 40/46 reference and real-money execution OFF.

No independent validation, no payout claim and no live-order authorization are created by Jeu 47.
