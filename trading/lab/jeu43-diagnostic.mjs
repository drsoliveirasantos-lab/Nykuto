...
import {filterNormalizedStreams43} from './jeu43-filter.mjs';
import {allNormalizedContexts43} from './jeu43-context.mjs';
...
export function runStudy43(bundle,mnq,prior,onProgress=()=>{}){
 ...
 const views=[],runs=[],audit={controls:0,prefixes:0,filterPrefixes:0,contextPrefixes:0,normalizationPrefixes:0,checkedTrades:0,passed:false};
 for(const month of JEU40_MONTHS){
  ...
  const contexts=contexts40(markets,month),original=filtered40(markets,contexts,month,available),normalizations=allNormalizedContexts43(markets,original.streams);
  for(const variant of JEU43_VARIANTS){const costs={};
   for(const[cost,factor]of [['normal',1],['stress',2]]){
    const filtered=filterNormalizedStreams43(original.streams,normalizations,factor,variant.id),run=simulateConfidencePortfolio(filtered.streams,contexts,period,factor,'fixed100');
    ...
    for(const {date:day}of available){
     const prefixContexts=contexts40(markets,month,day),before=filtered40(markets,prefixContexts,month,available,day),prefixNormalizations=allNormalizedContexts43(markets,before.streams,day),prefix=filterNormalizedStreams43(before.streams,prefixNormalizations,factor,variant.id),end=new Date(Date.parse(day+'T00:00Z')+86400000).toISOString().slice(0,10);
     ...
     for(const[symbol,cs]of prefixNormalizations)same([...cs],[...normalizations.get(symbol)].filter(([,c])=>c.day<=day),'Future normalization leak');audit.normalizationPrefixes++;
    ...
    for(const t of run.trades){
     ...
     if(variant.targetSymbol===t.symbol)ensure(filtered.decisions.some(d=>d.symbol===t.symbol&&d.time===t.entryTime&&d.side===t.side&&d.allowed),'Executed trade violates normalized veto');audit.checkedTrades++;
    }
    ...
    runs.push({month:month.id,variant:variant.id,factor,period,coverage:cov,run,decisions:filtered.decisions,normalizations:Object.fromEntries([...normalizations].map(([symbol,values])=>[symbol,[...values.values()]]))});
    onProgress({completed:runs.length,total:48,month:month.id,variant:variant.id,cost,audit:{...audit}});
   }
   views.push({month:month.id,label:month.label,variant:variant.id,mode:'funded',resetAtStart:true,period,coverage:cov,costs});
  }
 }
 ensure(runs.length===48&&audit.controls===16&&audit.prefixes===984&&audit.filterPrefixes===984&&audit.contextPrefixes===984&&audit.normalizationPrefixes===984,'Wrong replay/audit count');audit.passed=true;
 ...
 const summaries=Object.fromEntries(JEU43_VARIANTS.map(v=>[v.id,Object.fromEntries(['normal','stress'].map(cost=>[cost,summarize40(views.filter(x=>x.variant===v.id),cost)]))]));
 return {report:{schema:'jeu43-normalized-entry-report-v1',policy:JEU43_POLICY,variants:JEU43_VARIANTS,newConfigurations:2,executionCount:48,coverage,views,summaries,audit,reviews:Object.fromEntries(JEU43_VARIANTS.filter(v=>v.targetSymbol).map(v=>[v.id,review43(cellsFor(v.id))])),sourceMethods:{normalization:'Kronos-style past window; original JS implementation',news:'unobservable-no-point-in-time-archive',rsi:'unchanged-reference',volumeVeto42:'not-adopted'},selection:null,independent:false,confirmed:false,executionAllowed:false},privateRuns:{schema:'jeu43-private-v1',runs}};
}
