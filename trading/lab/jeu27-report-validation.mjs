import {REENTRY_REPORT as p} from './jeu27-public.mjs';
export async function verifyReentryReport(buffer){
  ...
  if(r.schema!==p.schema||r.freezeSha256!==p.freezeSha256||r.protocolSha256!==p.protocolSha256||r.selectionSha256!==p.selectionSha256||r.confirmed!==false||r.policy.independent!==false||['brokerEnabled','paperEnabled','shadowEnabled','liveFeed'].some(k=>r.policy[k]!==false)||r.audit.passed!==true||r.results.length!==4||r.selection.id!==null||r.selection.attempted!==4||r.selection.cumulativeAttempts!==61||r.holdout.status!=='not-opened'||r.policy.dailyLoss!==300||r.policy.riskMode!=='fixed-admission-cap')throw new Error('Invalid research state');
  const ids=new Set();
  for(const x of r.results){
    if(ids.has(x.id)||!['MNQ','MES','MYM','MGC'].includes(x.symbol)||x.id!==x.symbol+'/fresh-reentry150'||x.riskPerTrade!==150||x.dailyLoss!==300||x.confirmed!==false||x.windows.length!==2||x.checks.length!==8||x.researchPassed!==x.checks.every(c=>c.pass)||x.researchPassed!==false)throw new Error('Invalid candidate state');ids.add(x.id);
    for(const cost of ['normal','stress']){
      const d=x.diagnostic[cost];
      if(!Number.isInteger(d.repeatedSideTrades)||d.repeatedSideTrades<0||d.repeatedSideTrades>d.trades||!Number.isFinite(d.repeatedSideNet)||Math.abs(d.dailyMean-Math.round(d.net/d.daily.observed*100)/100)>1e-7)throw new Error('Invalid daily or repeat totals');
