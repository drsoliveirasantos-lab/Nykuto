...
import {JEU28_POLICY as Q,JEU28_PRODUCTS,JEU28_SCENARIOS} from './jeu28-policy.mjs';
...
const validProduct=p=>JEU28_PRODUCTS.some(x=>x.symbol===p.symbol&&x.tick===p.tick&&x.multiplier===p.multiplier&&x.fees===p.fees);
export function simulateBreakEven(candles, signals, scenario, product, { start, end }, costFactor = 1, account = true) {
  if (![1, 2].includes(costFactor) || !JEU28_SCENARIOS.some(s => s.id === scenario.id && scenario.guarded === true) || !validProduct(product)) throw new Error('Invalid simulation policy');
  ...
  const close = (fill, bar) => {
    ...
    if (position.breakEvenAt !== null && ['Stop', 'Stop gap'].includes(fill.reason)) fill = {...fill, reason: fill.reason === 'Stop' ? 'Break-even stop' : 'Break-even gap'};
  ...
  for (const bar of candles) {
    ...
    if (!position && signal && entries < Q.maxTradesPerDay && losses < RULES.lossStreak && realizedR > -RULES.maxDailyLoss) {
      ...
      position = { ticker: bar.ticker, side: signal.side, day, priorRefusals:refused[signal.side], entryTime: bar.time, entry: bar.open,
        ...
        initialStop: cents(bar.open - sign * terms.risk), breakEvenAt: null,
        balanceBefore: balance, dayStart, floorBefore: account ? floor : null };
      consumed.add(signal.side);entries++;
    }
    if (position) { const fill = riskFill(product, position, bar, balance, floor, dayStart, scenario.guarded, account, riskPolicy); if (fill) close(fill, bar); else { const protection = closedBreakEven(position, bar, product); if (protection) Object.assign(position, protection); } }
  }
