...
// One pass through closed candles. No indicator, prior day or future candle.
export function openingSignals(candles, product) {
  const result=new Map();let day='',opening=[],range=null,pending=null,seen=new Set(),previous=null;
  for(const b of candles){
    if(previous&&b.time<=previous.time)throw new Error('Nonchronological opening bars');
    if(b.day!==day){day=b.day;opening=[];range=null;pending=null;seen=new Set();previous=null;}
