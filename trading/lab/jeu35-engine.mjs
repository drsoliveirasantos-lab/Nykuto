// Game35 derivative of frozen Game34. Only a named market target changes.
...
import { accountProfile, accountFloor, riskCap, consistencyStatus } from './jeu33-policy.mjs';
import { accountSizedTerms, accountRiskGate } from './jeu33-risk.mjs';
import { JEU35_POLICY as P, exitProfile, exitTerms } from './jeu35-policy.mjs';
...
function validatedStreams(streams, period) {
  ...
  if (![period.start, period.end].every(d => typeof d === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(d))
    ...
    || period.start >= period.end || period.start < P.from || period.end > P.end) throw new Error('Authorized June–August period required');
...
export function simulateExitPortfolio(streams, period, costFactor = 1, mode = 'funded', variant = 'baseline') {
  exitProfile(variant);
  if (!['evaluation', 'funded'].includes(mode)) throw Error('Invalid account stage');
  const profileId = '50k-reduced100';
  const A = accountProfile(profileId), policy = A, account = true;
  ...
  const finishDay = () => {
    ...
    const payout = payoutEligibility({balance, initial:A.initial, qualifyingDays, mode, breached:status==='breached'});
    ...
    if (mode === 'funded' && payout.goalEligible && status !== 'breached') {
      ...
      goalDay = day; status = 'personalGoalMet'; terminalDay = day;
    }
    ...
    if (mode === 'evaluation' && status !== 'breached' && consistencyStatus(bestDay, profit, A).passed) {
  ...
  for (let i = 0; i < ordered[0].candles.length; i++) {
    ...
    if (day !== clock.day) {
      finishDay(); if (['targetMet', 'personalGoalMet', 'breached'].includes(status)) break;
    ...
    for (const s of ordered) {
      ...
      cap = riskCap(balance, floor, cap, A);
      const decision = accountSizedTerms(signal, bar.open, bar.time, s.product, costFactor, cap);
      if (decision.blocked) { reject(s.symbol, bar.time, decision.blocked); continue; }
      const terms = exitTerms(decision.terms, s.symbol, variant), blocked = accountRiskGate({ balance, floor, dayStart, ...terms }, A, cap);
      ...
      position = { symbol: s.symbol, strategy: s.strategy, ticker: bar.ticker, side: signal.side, day,
        ...
        riskCapUSD: cap, balanceBefore: balance, dayStart, floorBefore: account ? floor : null };
  ...
  return { profileId, mode, initial: A.initial, withdrawnUSD, receiptEUR, goalDay, personalGoalAchieved:goalDay!==null,
