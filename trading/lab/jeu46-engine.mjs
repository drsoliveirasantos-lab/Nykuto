...
import {timeExitDecision45} from './jeu45-time-exit.mjs';
...
export function simulateConfidencePortfolio(streams, contexts, period, costFactor = 1, profileId = 'fixed100', timeExitSymbol = null, invalidationSymbol = null) {
  if(profileId!=='fixed100'||![null,'MNQ'].includes(timeExitSymbol)||![null,'MNQ','MES'].includes(invalidationSymbol)||(timeExitSymbol&&invalidationSymbol))throw Error('Game46 fixed risk and isolated exit required');
  ...
  for (let i = 0; i < ordered[0].candles.length; i++) {
    ...
    if (position) {
      const s = ordered.find(s => s.symbol === position.symbol), bar = s.candles[i];
      const timedExit = position.timeExitAt === bar.time;
      const invalidationExit = position.invalidationAt === bar.time;
      const observedBar = (timedExit || invalidationExit) ? { ...bar, high: bar.open, low: bar.open } : bar;
      const fill = riskFill({ ...s.product, multiplier: s.product.multiplier * (position.quantity ?? 1) }, position, observedBar, balance, floor, dayStart, true, account, policy);
      if (fill) close(fill, bar);
      else if (timedExit) close({ price: bar.open, reason: 'Time exit 30m' }, bar);
      else if (invalidationExit) close({ price: bar.open, reason: 'Closed range invalidation' }, bar);
      else {
        const decision = invalidationSymbol ? invalidationDecision46(position, bar, invalidationSymbol) : timeExitDecision45(position, bar, s.product, timeExitSymbol);
        if (decision) Object.assign(position, decision);
      }
