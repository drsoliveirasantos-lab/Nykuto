...
import { JEU21_POLICY as P } from './jeu21-policy.mjs';
...
export function nativeSignals(candles, thirty, mode='pullback') {
  if(!['pullback','cross'].includes(mode))throw new Error('Unregistered native-preparation method');
  const ctx=contextFor(thirty), result=new Map();let j=-1;
  for(const [time,signal] of signalsFor(candles,5,mode)){
...
export function inspectNativePreparation(bundle,product){
  if(!['MES','MYM','MGC'].includes(product.symbol))throw new Error('Unregistered market');
  ...
  for(const [index,d] of JEU19_SEGMENTS.filter(d=>d.symbol===product.symbol).entries()){
