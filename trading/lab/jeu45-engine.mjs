...
export function simulateConfidencePortfolio(streams, contexts, period, costFactor = 1, profileId = 'fixed100', timeExitSymbol = null) {
  if(profileId!=='fixed100'||![null,'MNQ','MES'].includes(timeExitSymbol))throw Error('Game45 fixed risk and isolated market required');
  ...
  for (let i = 0; i < ordered[0].candles.length; i++) {
    ...
    if (position) {
      ...
      const observedBar = timedExit ? { ...bar, high: bar.open, low: bar.open } : bar;
      ...
      if (fill) close(fill, bar);
        ...
        const decision = timeExitDecision45(position, bar, s.product, timeExitSymbol);
