...
const $=id=>document.getElementById('reentry'+id);
...
function show(){
  ...
  for(const x of report.results){
    ...
    row($('Changes'),[x.symbol,d.repeatedSideTrades,money(d.repeatedSideNet),c.unchangedCount,c.addedCount,c.removedCount,money(c.deltaNet)]);
...
async function load(){
  try{
    const res=await fetch('./jeu27-report.json',{credentials:'same-origin',cache:'no-store',redirect:'error',signal:AbortSignal.timeout(15000)});
    if(!res.ok||!res.headers.get('Content-Type')?.includes('application/json'))throw new Error('Unavailable');
    report=await verifyReentryReport(await res.arrayBuffer());
    $('Status').textContent='Aucune candidate validée';
    $('Message').textContent='Les quatre configurations échouent. MNQ atteint 42 trades, mais les six entrées supplémentaires retirent 431 $ au total normal et une séance reste absente. Aucune candidate sélectionnée ; réserve fermée.';
