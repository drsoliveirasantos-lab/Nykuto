...
import {JEU26_POLICY as Q,JEU26_PRODUCTS,JEU26_SCENARIOS} from './jeu26-policy.mjs';
import {riskGate,riskFill} from './jeu23-risk.mjs';
import {failedBreakoutTerms,failedRiskProfile} from './jeu26-terms.mjs';
const cents=n=>Math.round(n*100)/100;
const validProduct=p=>JEU26_PRODUCTS.some(x=>x.symbol===p.symbol&&x.tick===p.tick&&x.multiplier===p.multiplier&&x.fees===p.fees);
export function simulateFailure(candles, signals, scenario, product, { start, end }, costFactor = 1, account = true) {
  if (![1, 2].includes(costFactor) || !JEU26_SCENARIOS.some(s => s.id === scenario.id && scenario.guarded === true) || !validProduct(product)) throw new Error('Invalid simulation policy');
  const riskPolicy=failedRiskProfile(scenario);
  const trades = [], days = [], denied = { tradeRisk: 0, dailyBudget: 0, floorReserve: 0, missingPivot: 0, invalidPivot: 0, netReward: 0, invalidStop:0, outsideRange:0, sideLimit:0 };
  ...
  for (const bar of candles) {
    ...
    if (!position && signal && entries < RULES.maxTrades && losses < RULES.lossStreak && realizedR > -RULES.maxDailyLoss) {
      ...
      signalCount++;
      const decision = failedBreakoutTerms(signal, bar.open, bar.time, product, costFactor);
      ...
      position = { ticker: bar.ticker, side: signal.side, day, priorRefusals:refused[signal.side], entryTime: bar.time, entry: bar.open,
        ...
        rangeClosedAt:signal.rangeClosedAt,rangeHigh:signal.rangeHigh,rangeLow:signal.rangeLow,breakoutAt:signal.breakoutAt,stopPrice:signal.stopPrice,pattern:signal.pattern,excursionExtreme:signal.excursionExtreme,
