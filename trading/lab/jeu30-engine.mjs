...
import { JEU30_POLICY as P, JEU29_PROFILES, JEU29_PRODUCTS, JEU29_RISK } from './jeu30-policy.mjs';
...
function validatedStreams(streams, period) {
  ...
  if (![period.start, period.end].every(d => typeof d === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(d))
    ...
    || period.start >= period.end || period.start < P.from || period.end > P.end) throw new Error('Authorized August period required');
...
export function simulateAugustPortfolio(streams, period, costFactor = 1, account = false) {
