...
import {JEU25_POLICY as Q,JEU25_PRODUCTS,JEU25_SCENARIOS} from './jeu25-policy.mjs';
import {riskFill} from './jeu23-risk.mjs';
import {gradeSignals,gradedRiskGate,gradedProfile} from './jeu25-risk.mjs';
import {openingTerms} from './jeu22-engine.mjs';
const cents=n=>Math.round(n*100)/100;
const validProduct=p=>JEU25_PRODUCTS.some(x=>x.symbol===p.symbol&&x.tick===p.tick&&x.multiplier===p.multiplier&&x.fees===p.fees);
export function simulateGraded(candles, signals, scenario, product, { start, end }, costFactor = 1, account = true) {
  if (![1, 2].includes(costFactor) || !JEU25_SCENARIOS.some(s => s.id === scenario.id && scenario.guarded === true) || !validProduct(product)) throw new Error('Invalid simulation policy');
  const riskPolicy=gradedProfile(scenario),grades=gradeSignals(candles,signals,product).grades;
  ...
  for (const bar of candles) {
    ...
    if (!position && signal && entries < RULES.maxTrades && losses < RULES.lossStreak && realizedR > -RULES.maxDailyLoss) {
      ...
      const grade=grades.get(bar.time);if(!grade)throw new Error('Missing causal risk grade');
      const blocked = gradedRiskGate({ balance, floor, dayStart, ...terms, account },grade.cap,riskPolicy);
      if (blocked) { denied[blocked]++;refused[signal.side]++; continue; }
      const sign = signal.side === 'Long' ? 1 : -1;
      position = { approvedRiskCap:grade.cap,contextScore:grade.score,contextMissing:grade.missingContext,ticker: bar.ticker, side: signal.side, day, priorRefusals:refused[signal.side], entryTime: bar.time, entry: bar.open,
