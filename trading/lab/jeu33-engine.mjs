// Game33 derivative of pinned Game32. Entry ordering and pessimistic fills stay identical.
// Four account profiles change only sizing, EOD floor and evaluation target.
...
import { JEU33_POLICY as P, accountProfile, accountFloor, riskCap, consistencyStatus } from './jeu33-policy.mjs';
...
export function simulateAccountPortfolio(streams, period, costFactor = 1, profileId = '50k-reduced100') {
  ...
  const finishDay = () => {
    ...
    if (account && status !== 'breached' && consistencyStatus(bestDay, profit, A).passed) {
  ...
  return { profileId, initial: A.initial, consistency: consistencyStatus(bestDay, net, A), status, terminalDay, net, balance, floor: account ? floor : null,
