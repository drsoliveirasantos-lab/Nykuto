...
import {JEU27_POLICY as Q,JEU27_PRODUCTS,JEU27_SCENARIOS} from './jeu27-policy.mjs';
import {riskGate,riskFill,riskProfile} from './jeu23-risk.mjs';
import {openingTerms} from './jeu22-engine.mjs';
const cents=n=>Math.round(n*100)/100;
const validProduct=p=>JEU27_PRODUCTS.some(x=>x.symbol===p.symbol&&x.tick===p.tick&&x.multiplier===p.multiplier&&x.fees===p.fees);
export function simulateReentry(candles, signals, scenario, product, { start, end }, costFactor = 1, account = true) {
  if (![1, 2].includes(costFactor) || !JEU27_SCENARIOS.some(s => s.id === scenario.id && scenario.guarded === true) || !validProduct(product)) throw new Error('Invalid simulation policy');
  const riskPolicy=riskProfile({...scenario,id:'admission-risk150'});
  const trades = [], days = [], denied = { tradeRisk: 0, dailyBudget: 0, floorReserve: 0, missingPivot: 0, invalidPivot: 0, netReward: 0, invalidStop:0, returnedInside:0, staleRepeat:0 };
  let balance = P.initial, floor = P.initial - P.maxLoss, position = null, day = '', dayStart = balance;
  let consumed=new Set(),lastExitClose={},refused={Long:0,Short:0};
  ...
  const close = (fill, bar) => {
    ...
    lastExitClose[position.side] = bar.time + 300;
    position = null;
  ...
  for (const bar of candles) {
    ...
    if (day !== bar.day) {
      ...
      day = bar.day; dayStart = balance; entries = 0; losses = 0; realizedR = 0; consumed=new Set();lastExitClose={};refused={Long:0,Short:0};
    }
    ...
    if (!position && signal && entries < Q.maxTradesPerDay && losses < RULES.lossStreak && realizedR > -RULES.maxDailyLoss) {
      ...
      if(consumed.has(signal.side) && (!Number.isFinite(signal.breakoutAt) || signal.breakoutAt - 300 < lastExitClose[signal.side])){denied.staleRepeat++;continue;}
      signalCount++;
      ...
      position = { ticker: bar.ticker, side: signal.side, day, entryNumber:entries+1, repeatedSide:consumed.has(signal.side), previousSameSideExitClose:lastExitClose[signal.side]??null, priorRefusals:refused[signal.side], entryTime: bar.time, entry: bar.open,
