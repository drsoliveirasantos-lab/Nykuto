// Isolated factorial replay; earlier frozen engines remain unchanged.
// Both existing cells are checked against their archived Jeu 16 trades and days.
...
import { JEU17_POLICY as A, JEU17_SCENARIOS } from './jeu17-policy.mjs';
const product = PRODUCTS.find(p => p.symbol === 'MNQ');
const cents = n => Math.round(n * 100) / 100;
function pivotTerms(signal, entry, entryTime, costFactor = 1) {
...
export function ablationTerms(signal, entry, entryTime, scenario, costFactor = 1) {
  const fixed = JEU17_SCENARIOS.find(s => s.id === scenario.id);
  if (!fixed || fixed.stopMode !== scenario.stopMode || fixed.netMargin !== scenario.netMargin || scenario.guarded !== true || ![1, 2].includes(costFactor)) throw new Error('Invalid ablation policy');
  const decision = scenario.stopMode === 'pivot' ? pivotTerms(signal, entry, entryTime, costFactor) : { terms: tradeTerms(product, signal.atr, costFactor) };
  if (decision.blocked) return decision;
  const { riskDollars, targetDistance, costDollars } = decision.terms;
  const ratio = (targetDistance * P.multiplier - costDollars) / (riskDollars + costDollars);
  if (scenario.netMargin && ratio < A.minNetRewardRisk) return { blocked: 'netReward' };
  return decision;
}

export function simulateAblation(candles, signals, scenario, { start, end }, costFactor = 1, account = true) {
  if (![1, 2].includes(costFactor) || !JEU17_SCENARIOS.some(s => s.id === scenario.id && s.stopMode === scenario.stopMode && s.netMargin === scenario.netMargin && scenario.guarded === true)) throw new Error('Invalid simulation policy');
  ...
  for (const bar of candles) {
    ...
    if (!position && signal && entries < RULES.maxTrades && losses < RULES.lossStreak && realizedR > -RULES.maxDailyLoss) {
      ...
      signalCount++;
      const decision = ablationTerms(signal, bar.open, bar.time, scenario, costFactor);
