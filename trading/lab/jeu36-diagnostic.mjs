...
import {confirmMarketStreams} from './jeu36-entries.mjs';
import {tradeStatistics} from './jeu34-diagnostic.mjs';
import {compareEntryRuns} from './jeu36-comparison.mjs';
import {JEU36_POLICY as P,JEU36_VARIANTS} from './jeu36-policy.mjs';
...
export function runEntryDiagnostic(bundle,mnq,prior){
 ...
 for(const month of JEU34_MONTHS){
  ...
  for(const variant of JEU36_VARIANTS){
   const available=base.map(s=>s.symbol===P.excluded?{...s,signals:new Map()}:s),confirmed=confirmMarketStreams(available,contexts,variant.id),streams=confirmed.streams;
   for(const mode of ['evaluation','funded']){
    ...
    for(const [cost,factor]of [['normal',1],['stress',2]]){
     ...
     if(variant.control){
      const old=prior.runs.filter(r=>r.month===month.id&&r.variant==='baseline'&&r.mode===mode&&r.factor===factor);ensure(old.length===1,'Missing control');
     ...
     for(const d of run.days){
      ...
      const sourcePrefix=available.map(s=>({...s,candles:s.candles.filter(b=>b.day<=d.day),signals:new Map([...s.signals].filter(([,v])=>v.day<=d.day))})),rebuilt=confirmMarketStreams(sourcePrefix,contexts,variant.id);
      for(const s of rebuilt.streams)same([...s.signals],[...streams.find(x=>x.symbol===s.symbol).signals].filter(([,v])=>v.day<=d.day),'Future confirmation leak');
      const prefix=simulateMonthlyPortfolio(rebuilt.streams,{start:period.start,end},factor,mode);
     ...
     for(const t of run.trades){
      ...
      const sign=t.side==='Long'?1:-1,rr=2;
      if(t.symbol===variant.symbol){ensure(t.entryTime===t.originalSignalClose+300,'Entry not delayed exactly one bar');ensure(t.stop===t.stopPrice,'Original stop moved');}
     ...
     costs[cost]={...tradeStatistics(run.trades),status:run.status,terminalDay:run.terminalDay,personalGoalAchieved:run.personalGoalAchieved,goalDay:run.goalDay,receiptEUR:run.receiptEUR,withdrawnUSD:run.withdrawnUSD,balance:run.balance,floor:run.floor,headroom:cents(run.balance-run.floor),drawdown:run.drawdown,calendar,payout:last?.payout??null,daily:run.daily,denied:run.denied,
      ...
      comparison:compareEntryRuns(prior.runs.find(r=>r.month===month.id&&r.variant==='baseline'&&r.mode===mode&&r.factor===factor).run,run),
      confirmation:{candidates:confirmed.decisions.length,accepted:confirmed.decisions.filter(d=>d.allowed).length,reasons:confirmed.decisions.reduce((a,d)=>(a[d.reason]=(a[d.reason]||0)+1,a),{})},
      lossOnEntryBar:run.trades.filter(t=>t.netDollars<0&&t.entryTime===t.exitTime).length,executionAllowed:false};
     runs.push({month:month.id,variant:variant.id,mode,factor,period,run,confirmationDecisions:confirmed.decisions});
    }
