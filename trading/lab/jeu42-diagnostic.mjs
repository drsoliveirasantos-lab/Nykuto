...
import {filterPullbackStreams42} from './jeu42-pullback-volume.mjs';
import {JEU42_POLICY,JEU42_VARIANTS} from './jeu42-policy.mjs';
const cents=n=>Math.round(n*100)/100,ensure=(x,m)=>{if(!x)throw Error(m);},same=(a,b,m)=>ensure(JSON.stringify(a)===JSON.stringify(b),m);
function decisionCounts(decisions){return {signals:decisions.length,blocked:decisions.filter(d=>!d.allowed).length,phaseReasons:Object.fromEntries([...new Set(decisions.filter(d=>d.detail).map(d=>d.detail.phaseStatus))].map(reason=>[reason,decisions.filter(d=>d.detail?.phaseStatus===reason).length])),byMarket:JEU29_PRODUCTS.map(p=>({symbol:p.symbol,signals:decisions.filter(d=>d.symbol===p.symbol).length,blocked:decisions.filter(d=>d.symbol===p.symbol&&!d.allowed).length}))};}
export function review42(cells){
...
export function runStudy42(bundle,mnq,prior){
 ...
 for(const month of JEU40_MONTHS){
  ...
  for(const variant of JEU42_VARIANTS){const costs={};
   for(const[cost,factor]of [['normal',1],['stress',2]]){
    const filtered=filterPullbackStreams42(original.streams,contexts,factor,variant.id),run=simulateConfidencePortfolio(filtered.streams,contexts,period,factor,'fixed100');
    ...
    for(const {date:day}of available){
     const prefixContexts=contexts40(markets,month,day),before=filtered40(markets,prefixContexts,month,available,day),prefix=filterPullbackStreams42(before.streams,prefixContexts,factor,variant.id),end=new Date(Date.parse(day+'T00:00Z')+86400000).toISOString().slice(0,10);
    ...
    for(const t of run.trades){
     ...
     if(variant.id==='mes-pullback-volume'&&t.symbol==='MES')ensure(filtered.decisions.some(d=>d.symbol===t.symbol&&d.time===t.entryTime&&d.side===t.side&&d.allowed&&!d.detail.phaseVeto),'Executed MES violates frozen phase veto');audit.checkedTrades++;
    }
 ...
 const cells=views.filter(v=>v.variant==='mes-pullback-volume').flatMap(v=>['normal','stress'].map(cost=>({month:v.month,cost,net:v.costs[cost].net,delta:v.costs[cost].comparison.delta,drawdown:v.costs[cost].drawdown,referenceDrawdown:views.find(b=>b.month===v.month&&b.variant==='baseline').costs[cost].drawdown,status:v.costs[cost].status})));
 const summaries=Object.fromEntries(JEU42_VARIANTS.map(v=>[v.id,Object.fromEntries(['normal','stress'].map(cost=>[cost,summarize40(views.filter(x=>x.variant===v.id),cost)]))]));
 return {report:{schema:'jeu42-pullback-volume-report-v1',policy:JEU42_POLICY,variants:JEU42_VARIANTS,newConfigurations:1,executionCount:32,coverage,views,summaries,audit,review:review42(cells),selection:null,independent:false,confirmed:false,executionAllowed:false},privateRuns:{schema:'jeu42-private-v1',runs}};
}
