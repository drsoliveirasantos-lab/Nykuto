...
export function makePrompt(ctx) {
  return `Analyse pédagogique de captures de futures pour une simulation manuelle Nykuto.\nContexte déclaré, non vérifié : ${JSON.stringify(ctx)}\nJe joins les images séparément. Ne prétends pas les avoir vues si elles manquent. Décris la tendance et la structure visibles, les niveaux lisibles et les indicateurs affichés. Si le RSI est visible, distingue momentum, zones extrêmes et régime de tendance ; 70/30 ne sont pas des ordres. Décris une divergence régulière ou cachée uniquement avec deux pivots prix/RSI lisibles et déjà confirmés, en précisant quand leur confirmation devient disponible ; sinon écris non vérifiable. Ne confonds pas divergence et retournement confirmé. Pour les annonces, exige source primaire, heure, fuseau et distinction valeur publiée/consensus/précédent/révision. Sans information datée fournie ou vérifiée, annonce inconnue : ne jamais conclure à absence de news. Les scénarios Kronos éventuellement joints sont des hypothèses numériques, pas des probabilités calibrées et ne justifient aucune hausse du risque. Sépare observations, hypothèses et informations manquantes. Donne des scénarios conditionnels haussier/baissier/attente, leurs déclencheurs et invalidations. N'invente ni cours actuel, ni carnet, ni volume, ni news, ni taux de confiance. Signale les prix illisibles et les captures anciennes. Demande de confirmer manuellement le contrat, l'heure et les niveaux avant un ticket. Aucun ordre à exécuter.`;
}
